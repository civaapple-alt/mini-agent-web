...
Web Studio 将模型回答作为消息流的主内容。当前 assistant 执行段运行时，思考和工具活动逐项显示，
不合并成摘要。后续执行段成为当前段后，前一段中已完成的相邻成功活动才合并为可展开摘要；
Turn 结束后，最后一段的成功活动也按同一规则收拢。模型给出的中间进度说明始终可见。失败、
...
页面底部显示“有新活动 · 查看当前 Turn”。点击后，Studio 将当前 Turn 最后一条 assistant 消息滚到可视区底部。
如果本轮还没有 assistant 活动，则回到该 Turn 的输入。点击 Turn 轨道节点仍会定位到输入位置。
当前执行段运行时保留逐块活动顺序；
下一个执行段开始后，前一段的成功思考和工具活动才归入摘要。Turn 结束后，最后一段也会收拢；
