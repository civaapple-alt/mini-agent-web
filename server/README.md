...
- **🔄 双向通信桥接**：
  - **REST API**：提供线程管理、工作流控制（Plan/Goal）、环境探测（WorldState/MCP）、配置变更及审批响应的标准 HTTP 路由。
  - **WebSocket 网关 (`/ws/events`)**：将 `mini-agent-app-server` 的标准输出流实时分发给连接的前端客户端，支持多客户端订阅与状态同步。
...
└── routes/
    ├── __init__.py      # 路由聚合
    ├── agent.py         # 轮次提交、实时流式推理、纠偏与中断路由 (/api/agent/*)
    ├── threads.py       # 会话线程创建、分支分叉、读取与删除 (/api/threads/*)
    ├── world.py         # Plan Mode、Goal 任务、WorldState 与 MCP 诊断 (/api/world/*)
...
| `/ws/events` | `WebSocket` | 实时事件推流与双向协议交互通道 |
