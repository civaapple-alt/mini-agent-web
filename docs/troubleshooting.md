...
Web Studio 将模型回答作为消息流的主内容。当前 Turn 的活跃 block 保持展开；下一个 block 开始时，
执行中的思考或工具活动保持展开。Turn 结束后，相邻的成功思考和工具活动合并为可展开摘要；
模型给出的中间进度说明始终可见。失败、待审批和子任务委派活动单独显示，最终回复保持展开。
运行到完成以及重新打开历史时使用同一套 Turn 和 block 顺序规则，因此活动不会只因组件重新挂载
而改变默认展开状态。手动开合状态按 block ID 保存在当前页面内。
...
页面底部显示“有新活动 · 查看当前 Turn”；点击后才跳到当前轮。Turn 运行期间保留逐块活动顺序，
Turn 结束后，相邻的成功思考和工具活动按原顺序归入一个摘要。重新打开会话后仍使用相同分组，
完整输出仍可在详情中查看和复制。设置
