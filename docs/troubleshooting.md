...
前一个思考块会折叠为紧凑的单行摘要（图标、耗时、字数和展开控件），不再保留截断正文行；工具卡会收起已展开的输出。Turn 结束时会收起最后一个仍活跃的思考或工具 block；最终回复文本保持展开。
如果最终回复文本跟在一个思考或工具 block 后面，前一个 block 会在最终回复开始时折叠。
每个 block 仍留在原来的消息顺序中，不会因为思考结束而一起隐藏后续工具步骤。当前运行块以
“思考中”或“运行中”状态标识；已经过去的思考块保留可点击的摘要。点击“思考”标题可以手动展开
或收起单个思考块。
...
消息流左侧的 Turn 轨道按一次用户输入生成一个节点，普通输入、Goal 和实时纠偏使用来源标签
区分。轨道固定在聊天可视区中部，消息滚动和新 Turn 到来不会改变它的位置；历史节点过多时，
...
每个 block 只在后续 block 开始时折叠。重新打开会话后，属于同一 Turn 的连续 assistant 片段也会
合并，避免每次模型和工具往返都显示一张重复的摘要卡。完整输出仍可在详情中查看和复制。设置
...
委派给子代理的任务会按父 Turn 汇总在消息流中，卡片逐项显示任务名称、排队或运行状态
及最终状态。刷新或重新打开父会话后，状态从 Session 中的操作记录恢复。需要查看完整
输出时，在“运行详情”的“子任务”列表中打开已创建的子会话；创建子会话前失败的任务
仍会显示失败状态，但没有可打开的会话。委派子任务不占项目侧栏的普通会话列表，普通
派生会话仍会显示。

Child 可用 `task_report` 回报有界进展；Gateway 在工具成功结束后，以该报告实际所属的
尝试次数写入 Child Session。父代理通过 `task_read` 增量读取报告，并可用 `task_control` 修改或取消
排队任务、引导或取消运行任务、重试失败任务、取消顺序组，再用 `delegate_task` 增加
新任务。控制请求由 Gateway 后台执行后，会把已应用、部分成功、失败、跳过或目标过期的
结果合并回父代理的待处理唤醒，并带上有界的 Child ID 摘要；RPC 异常的原文不会注入模型上下文。
父代理应先通过 `task_read` 重新检查 App Server 的 Child 操作投影，再决定后续动作。
报告到达时，Gateway 会合并同一父会话的待处理唤醒；父 Turn 正在运行时等待
安全续行边界，父 Turn 空闲时自动续行一轮。若状态与运行面板暂时不一致，刷新父会话
和“运行详情”；若子任务没有可打开入口，通常表示它在创建 Child Session 前失败，具体
诊断仍显示在批次卡和子任务列表中。

Gateway 重启后仍可从 Child Session 读取已保存报告；重启前尚未送达父代理的唤醒不会
自动重放。重新打开父会话后，可继续让父代理通过 `task_read` 检查子任务报告。
