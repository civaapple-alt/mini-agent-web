...
- **🛡️ 交互式安全审批与记忆闭环**：
  - 当 Agent 尝试执行敏感操作（如 Shell 命令或 `apply_patch`）时，InputBar 底部常驻悬浮审批 Dock；
...
- **📊 侧边栏工作流面板 (SidePanel)**：
  ...
  - **Builtin Tools 权限控制**：默认暴露少量高价值工具（`read_file`, `apply_patch`, `shell`, `read_image`），`web_fetch` 需显式启用；`write_file` 与 `edit_file` 不再存在。
...
- **REST API**：通过 `src/api.js` 调用后端 `/api/threads`、`/api/threads/{thread_id}/items`、`/api/threads/{thread_id}/settings`、`/api/threads/{thread_id}/goal`、`/api/workflows/state`（只读聚合）、`/api/world/*`、`/api/projects/*`、`/api/settings`。
- **WebSocket**：连接 `/ws/agent` 双向全双工通道，传输 Turn 提交、实时 Steering 纠偏、Interrupt 停止生成、安全审批、`item/started` / `item/completed` ThreadItem 生命周期和 Goal/settings runtime notifications。
