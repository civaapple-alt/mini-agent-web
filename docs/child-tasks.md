...
父代理可在子任务运行期间多次 steer 同一个子 Session。`retry` 只接受失败、取消或步数受限的任务，
并用上次保存的提示词启动新 Turn。它不会接收新的评审意见或修改要求。已完成的子 Session 当前不能
原地接收后续任务；父代理需要再调用 `delegate_task` 创建一个新子任务。列表和消息流按尝试次数分组显示
生命周期，并标出当前阶段、最新进展、顺序组位置和恢复异常。
