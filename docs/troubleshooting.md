...
文件的一页。完成后的卡片会自动展开有界结果，并在文件名后显示实际请求的 1-based 行范围；
结果头中的 `next_offset` 可直接用于读取下一页。若只看到文件名，请确认页面已更新到包含
该修复的 Web Studio 版本；旧版本需要手动展开“执行输出 (Output)”，并且可能无法识别
旧事件中的 `content` 返回字段。
...
如果在审批 Dock 出现时点击“停止”，停止请求会先撤销该 Project/Thread/Turn 下的所有
