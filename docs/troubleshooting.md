...
如果 steer 后仍显示上一段的“本轮未完整结束”，确认页面已更新到当前版本。活动 Turn 和 steer 续行期间会隐藏旧终态提示；新的 `turn_started` 会清除上一段结果，当前段结算后再显示新的失败、步数上限或中断信息。
...
Web Studio 将模型回答作为消息流的主内容。实时流会把同一 Turn 的多个执行段追加到一条 assistant 消息中。
当前执行段逐项显示；它保持展开，直到下一个执行段开始，此时前一段中相邻的成功思考和工具活动才合并为可展开摘要。
没有后续执行段的最后一段保持展开。模型给出的中间进度说明始终可见。失败、
待审批和子任务委派活动单独显示，最终回复保持展开。重新打开历史时使用相同的 Turn 和 block 顺序，
因此活动不会只因组件重新挂载而改变默认展开状态。手动开合状态按 block ID 保存在当前页面内。
...
### 信息流中的 Session Turn 方向感

消息流左侧的 Turn 轨道按一次用户输入生成一个节点，普通输入、Goal、手动纠偏和子代理续行使用来源标签
区分。子代理续行带有持久化的 `turnSource: "child_wakeup"` 元数据，Turn 轨道标记“子代理更新”；
内部唤醒不会作为用户消息显示。轨道固定在聊天可视区中部，消息滚动和新 Turn 到来不会改变它的位置；历史节点过多时，
可以在轨道内滚动。节点只代表 Turn，不代表 reasoning 或单个工具；点击节点会回到对应输入并
短暂高亮该轮。轨道作为独立叠加层显示，不占消息流的阅读列，正文仍保持居中宽度。
纠偏消息保留低对比的标签和中性色气泡，避免紫色强调盖过消息正文。移动端会隐藏轨道，键盘仍可聚焦和操作节点。

手动推进在本轮达到步数上限时，顶部状态栏和消息流末尾都会显示终态提示；消息流提示会说明
已保留当前检查点，并引导用户发送下一条指令继续推进。轨道以聊天视口的稳定中点为锚定位置，
流式 reasoning 内容增长不会改变其位置；“回到底部”按钮固定在聊天视口底部，不遮挡输入框。

每轮正文中的摘要卡只投影可靠事实：运行状态、已知步骤数、工具/命令次数、审批等待、失败或
中断信息，以及运行时明确提供的耗时。历史 Turn 缺少终态证据时显示“历史”，不会因为存在模型
回答就推断为已完成；当前版本也不会通过问号字符猜测“询问了几个问题”。

如果用户向上滚动查看旧内容，新的流式事件不会强制把页面拉到底部。摘要卡和 Turn 节点仍会更新，
页面底部显示“有新活动 · 查看当前 Turn”。点击后，Studio 将当前 Turn 最新活动或末尾定位到可视区底部，
而不是跳回该 Turn 的用户输入；只有本轮尚无 assistant 活动时才回到输入。点击 Turn 轨道节点仍会定位到输入位置。
重新打开会话后仍使用相同分组，完整输出仍可在详情中查看和复制。设置
`prefers-reduced-motion` 时不会使用持续旋转或高亮动画。

### 子代理消息流包含父会话历史或重复纠偏提示

子 Session 从父 checkpoint fork 时会继承模型上下文。这个 checkpoint 不是子 Session 自己的活动记录，
因此子智能体详情只显示该 child Session 本地持久化的输入、回复、思考和工具项。旧 child 没有本地活动项时，
页面显示空态，不回退显示父会话历史。来源 checkpoint 只作为详情元数据展示。

子代理报告和终态先更新父会话批次卡。父 Turn 运行时不会收到内部 `steer`；它空闲后，Gateway 合并待处理更新，
再启动一轮带 `child_wakeup` 来源的续行。手动 steer 只在 Gateway 收到 `steer_ack` 后提示一次。
如果仍看到子任务更新触发 steer 提示，确认 Gateway 和 Web Studio 已更新到同一版本。
Gateway 每轮最多向父代理提交 16 个子任务更新；最多缓存 64 个不同子任务的最新状态，
再将额外更新折叠为数量和最多 8 个示例 ID。队列摘要不替代持久状态，父代理可用 `task_read`
或运行面板读取最新状态。Gateway 重启会清除内存中的待处理唤醒；已保存的报告仍可从 App Server 读取。

右侧运行面板把“子智能体”作为独立顶层页。打开子任务后，详情在该页内展示并可返回列表；它不会替换父会话
消息流或运行状态页。无法打开的失败项没有对应 Child Session，请从任务卡查看创建失败诊断。
...
- **审批证据与完整命令**：`approval-evidence.jsonl` 保持有界，只记录审批类型、`call_id`、
  ...
  这属于有意保留的证据边界。
- **后台审批日志**：SDK 和 Gateway 日志只保留工具名、有限的 `apply_patch` 变更计数和请求 ID。
  Shell 的完整命令、文件路径以及其他自由文本摘要不会写入审批日志；需要查看详情时使用
  Web Studio 的审批卡或对应 Session item。
- **审批卡中的变更观测**：`apply_patch` 的审批卡会直接显示新增、修改、删除的文件数量，
  并可展开查看有界的目标路径。它只帮助判断操作范围，不展示完整补丁；完整参数和工具输出
  仍在工具详情中查看。删除文件会在摘要中明确标记，不能只看见 `apply_patch` 工具名就推断
  操作没有删除。
...
- Plan Mode 使用当前 Session 自有的 `plan/plan.md`；工具中的逻辑别名是 `plan.md`，
  与项目根目录的 `plan.md` 分开。“计划”页优先显示当前 Session 的计划，并在 Turn 完成后
  重新读取文件列表和内容。若仍看到旧内容，先确认当前项目与会话，再点“刷新”。
...
侧栏只显示真正存在活跃 Turn 的会话，并将其标为“运行中”。空闲会话不显示“已完成”、
“待命”、“已中断”或其他历史状态标签。SessionStore 进程在线但没有活跃 Turn 时，
不会被侧栏标为运行中；选中会话也不额外显示运行小圆点。切换运行中的会话和空闲会话
不会停止其他会话。

多个会话同时显示“运行中”是合法状态。不同 Project 即使使用同名 `default` Thread，
也不会互相贡献运行状态。若运行标记与实际不符，请检查 Session catalog 返回的
`turn_active` 和请求、WebSocket 携带的 `project_id`。`process_online`、锁状态和上一轮
结果仍可在运行详情中查看。
...
侧栏状态来自有界的会话 catalog：只有 `turn_active=true` 的会话显示“运行中”，其他
会话不显示历史结算标签。项目行的“活跃”数量也会随 catalog 刷新，而不是只使用项目
创建时的静态计数。运行详情仍提供失败、中断、步数上限、暂停和可恢复状态。

项目下的会话按 Session catalog 的最后活动时间从新到旧排列，标题下方显示“刚刚”、
“N 分钟前”、“N 小时前”或“N 天前”。更新时间缺失时会显示“时间未知”，并排在有
活动时间的会话之后；侧栏不会用当前时间代替缺失值。

### 查看代理子任务

委派任务的批次卡、子代理报告、运行详情入口和侧栏显示规则见
...
并重用原提示词。成功完成的子 Session 可通过 `task_control.assign` 接收评审意见，并在原 Session 开启新的
follow-up Turn；运行中或报告后仍运行的任务会 steer 当前 Turn。follow-up 可能因并发或顺序组限制先排队，
状态应保持为“排队中”，并在空位释放后自动启动。
