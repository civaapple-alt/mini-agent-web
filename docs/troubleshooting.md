...
Web Studio 将模型回答作为消息流的主内容。当前 Turn 执行时，思考和工具卡按发生顺序逐块显示。
思考停止更新后仍留在原位展开，后续工具和思考继续接在下面；当前 Turn 结束后，已完成的连续
思考和工具步骤才会折叠为活动摘要。运行中、等待审批、失败或中断的工具保持可见。点击摘要可以
展开过去的步骤，点击“思考”标题可以单独展开或收起思考内容。
...
Turn 结束后，已完成的思考和工具步骤会合并为可展开的步骤摘要。重新打开会话后，属于同一 Turn
的连续 assistant 片段也会合并，避免每次模型和工具往返都显示一张重复的摘要卡。完整输出仍可在详情中查看和复制。设置
