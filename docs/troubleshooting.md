...
前一个思考块和流式 assistant 文本块会折叠为紧凑摘要，工具卡会收起已展开的输出。策略按 block
逐步推进，不等整个 Turn 结束后才批量折叠。Turn 结束时会收起最后一个仍活跃的思考或工具 block；
最终回复文本保持展开。如果最终回复文本跟在其他 block 后面，前一个 block 会在最终回复开始时折叠。
